...
문서 전체를 실행 순서로 해석하지 않는다. 해당 작업의 절만 읽고 실행한다.
- 제품 실행·검증: 1절, 3절. 환경 변수·장애: 4~6절.
- 추천 실험 재현: 실험 ID나 script 이름으로 제목을 검색한다. 7절 이후는 개별 실험의 재현 기록이다.
- 현재 선택·다음 행동은 [현재 연구](../recommendation/active-experiment.md)를 따른다. 오래된 실험 제목의 분류 수를 현재 정책으로 사용하지 않는다.
- 명령의 경로·데이터는 원래 연구 환경 기준이다. 새 worktree에서는 데이터와 산출물의 실제 위치를 확인하며 재현 완료를 추측하지 않는다.
