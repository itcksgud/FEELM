...
전체 실행 전에 MovieLens Base-Train 영화 중 TMDB 링크가 있는 100편을 고정 hash 순서로 뽑아 다음을
실제로 확인했다.
...
그러나 100편이 잘 됐다고 전체 coverage가 통과했다고 말할 수는 없다. 다음 단계는 Base-Train 전체
69,603편을 같은 코드로 실행하고, 링크율·identity·결측률을 전체 분모로 다시 계산하는 것이다.

Locked Test와 추천 모델 비교는 아직 열지 않는다. 전체 특징 manifest가 통과한 뒤 Validation에서
인기도 기준선과 한 개의 개인화 후보를 고정해 비교한다.
