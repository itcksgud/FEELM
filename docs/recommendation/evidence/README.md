...
[REC-EV-019D Validation manifest](./manifests/rec-ev-019d-validation.json)는 결과 확인 전 생성한 계약·source
hash·protocol lock과 same-window Top-500·paired delta·strata·안전성 `FAIL`을 잠근다. Locked Test와
제품 정책은 바꾸지 않았고 `champion=null`이다.
...
[0.03681, 0.05462]`다. K5와 K10은 사용자·미래 구간이 달라 직접 비교하지 않는다. 019C full
Validation 전체에서 원시 양쪽 신호가 있어도 valid candidate anchor 부족으로 K5 97명, K10 46명이
fallback했으므로 양방향 신호
...
REC-EV-019D는 K10 Validation 1,479명에서 같은 K10 미래 창을 유지하고 profile만 first5·first10으로
바꿨다. tuning panel 합집합 426명을 제외한 1,053명의 common-seen-mask NDCG delta는 `+0.02656`,
95% CI `[0.01784, 0.03520]`였지만 Harm@2 증가 one-sided 95% upper가 `0.01235`로 사전 한계
`0.005`를 넘었다. 사전 우선순위에 따라 판정은 `FAIL_SAFETY_MARGIN_EXCEEDED`이며 K10이나 champion을
채택하지 않는다. 적용 strata `661/277/115`와 candidate-anchor loss `61/34`는 원자료 재계산과 일치했다.
여기서 `61/34`는 019D confirmatory 1,053명 값이며 019C full Validation의 `97/46`과 모집단이 다르다.
019D full-rescore 감사는 5,916개 ranking의 exact Top-10/Top-500, positive mean rank percentile,
aggregate를 확인했고 boundary tie는 0건이었다.

REC-EV-019E는 019D 결과를 본 뒤 같은 1,053명에서 새로 적용 가능한 277명만 K10으로 전환한 post-hoc
완화다. ΔNDCG `+0.013997 [0.008433, 0.019758]`, Harm upper `0.003799`로 Gate를 통과했지만
...
REC-EV-019F는 이 frozen routing을 동일 Validation 사용자의 더 뒤쪽 행에 적용한 별도 temporal episode다.
...
기대값으로 공개했고 실제 실행에서 일치했다. ΔNDCG는 `+0.003617 [0.000291, 0.007239]`, Harm upper는
`0.003741`이었다. 안전 한계는 넘지 않았지만 mean SESOI 0.005에 미달해 `INCONCLUSIVE`다. candidate
recall@500 `-0.012469`와 positive mean rank percentile `+0.020316` 악화도 비-Gate라는 이유로 숨기지 않았다.
독립 verifier는 802명 전원의 K5/K10 1,604개 full-catalog profile ranking을 exact Top-10/Top-500으로 재현했다.
