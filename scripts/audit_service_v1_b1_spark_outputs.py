...
TRAIN_NAMES = {
    ...
    "contract/feature-schema.v1.json", "implementation/service-v1-b1-spark-runner.md",
    "implementation/run_service_v1_b1_gbt.py", "implementation/service_v1_b1_spark_worker.py",
    "implementation/test_service_v1_b1_gbt_runner.py", "implementation/combination340_models.py",
...
class Roots:
    ...
    def predecessor_failure(self) -> Path:
        return self.standalone / "outputs/recommendation-evidence/service-v1-pretraining-20260913" / (PREDECESSOR_RUN_ID + "-preflight-failure.json")
    ...
    def sources(self) -> dict[str, Path]:
        ...
        result = {
            ...
            "implementation/service-v1-b1-spark-runner.md": self.standalone / "docs/recommendation/plans/service-v1-b1-spark-runner.md",
            "implementation/test_service_v1_b1_gbt_runner.py": self.standalone / "tests/test_service_v1_b1_gbt_runner.py",
        }
        for name in ("run_service_v1_b1_gbt.py", "service_v1_b1_spark_worker.py", "combination340_models.py", "rec046_common.py"):
            result["implementation/" + name] = self.standalone / "scripts" / name
        return result


SCORE_NAMES = {"contract/feature-schema.v1.json", "source/natural-score.parquet",
               "implementation/service-v1-b1-spark-runner.md", "implementation/run_service_v1_b1_gbt.py",
               "implementation/service_v1_b1_spark_worker.py", "implementation/test_service_v1_b1_gbt_runner.py",
               "runtime/docker-image-id"}
...
def controls_for(roots: Roots, phase: str) -> list[dict[str, Any]]:
    if phase == "preflight":
        path = roots.predecessor_failure()
        return [pin(path, roots.logical(path))]
    prior = "preflight" if phase == "fit" else "fit"
    b = roots.bundle(prior)
    return [pin(b / name, roots.logical(b / name)) for name in sorted(inventory(b))] + [pin(roots.review(prior), roots.logical(roots.review(prior)))]


def audit_recovery_reference(
    roots: Roots, manifest: dict[str, Any], expected_outer: str, expected_worker: str, limits: Limits
) -> dict[str, Any]:
    failure_path = roots.predecessor_failure()
    failure_pin = pin(failure_path, roots.logical(failure_path))
    if limits.canonical:
        need(
            (failure_pin["bytes"], failure_pin["sha256"])
    ...
    need(
        ...
        and failure.get("phase") == "preflight"
    ...
    need(
        ...
        and "masked bundle file set is not exactly the reviewed three files" in str(run.get("stdout", "")),
        "predecessor was not the reviewed mount-wiring failure",
    )
    reference_path = roots.bundle("preflight") / "recovery-reference.json"
    reference = json_object(reference_path)
    need(reference.get("schemaVersion") == "feelm-service-v1-b1-recovery-reference/1"
         and reference.get("status") == "PREDECESSOR_FAILURE_VERIFIED", "recovery reference schema/status")
    expected = {
        "predecessorRunId": PREDECESSOR_RUN_ID,
        "runId": RUN_ID,
        "predecessorFailure": failure_pin,
        "predecessorOuterRunnerSha256": PREDECESSOR_OUTER_RUNNER_SHA256,
        "predecessorSparkWorkerSha256": PREDECESSOR_SPARK_WORKER_SHA256,
        "outerRunnerSha256": expected_outer,
        "sparkWorkerSha256": expected_worker,
        "failureClass": "MASKED_BUNDLE_CONTAINER_MOUNT_WIRING",
        "modelFitPerformed": False,
        "fullPreflightPerformed": False,
    }
    for key, value in expected.items():
        need(reference.get(key) == value, "recovery reference drift: " + key)
    need(manifest.get("runId") == RUN_ID, "preflight run ID drift")
    need(manifest.get("predecessorFailureSha256") == failure_pin["sha256"], "manifest predecessor pin drift")
    need(manifest.get("recoveryReferenceSha256") == digest(reference_path), "manifest recovery reference pin drift")
    return {"predecessorFailure": failure_pin, "reference": pin(reference_path)}
...
def audit_lock(roots: Roots, phase: str, lock: dict[str, Any], manifest: dict[str, Any],
    ...
    score = phase == "score"
    schema = "feelm-service-v1-b1-score-input-lock/1" if score else "feelm-service-v1-b1-input-lock/1"
    source_key = "scoreSourceRecords" if score else "trainingSourceRecords"
    digest_key = "scoreSourceSetSha256" if score else "trainingSourceSetSha256"
    need(lock.get("schemaVersion") == schema and lock.get("phase") == phase, "input lock phase/schema")
    need(lock.get("evaluationTargetsRead") is False, "input lock permits evaluation targets")
    sources = record_map(lock.get(source_key))
    need(set(sources) == (SCORE_NAMES if score else TRAIN_NAMES), "source set is not exact")
    actual_records = []
    paths = roots.sources()
    if not score:
        masked_root = paths["source/tmdb-masked-train.parquet"].parent
        need(
            masked_root.is_dir()
            and not masked_root.is_symlink()
            and not (hasattr(masked_root, "is_junction") and masked_root.is_junction()),
            "masked bundle directory is missing or linked",
        )
        children = list(masked_root.iterdir())
        need(
        ...
        need(all(path.is_file() and not path.is_symlink() for path in children),
             "masked bundle contains a non-file")
    for name, record in sorted(sources.items()):
        if name == "runtime/docker-image-id":
            ...
            need(record == observed, "Docker ID virtual pin")
        else:
            observed = pin(paths[name], name)
            need(record == observed, "source file drift: " + name)
            if limits.canonical and name in PINNED:
                need((observed["bytes"], observed["sha256"]) == PINNED[name], "canonical source changed: " + name)
            if limits.canonical and name.endswith("service-v1-b1-spark-runner.md"):
                need(observed["sha256"] == PLAN_SHA256, "reviewed plan changed")
        actual_records.append(observed)
    outer = sources["implementation/run_service_v1_b1_gbt.py"]
    worker = sources["implementation/service_v1_b1_spark_worker.py"]
    need(outer["sha256"] == expected_outer and worker["sha256"] == expected_worker, "explicit implementation pins changed")
    ...
    controls = controls_for(roots, phase)
    same_records(lock.get("controlReferences"), controls, "control references changed or incomplete")
    for key, value in ((digest_key, set_digest(actual_records)),
                       ("controlReferenceSetSha256", set_digest(controls)),
                       ("inputSetSha256", set_digest(actual_records + controls))):
        need(lock.get(key) == value, "lock digest: " + key)
        need(manifest.get(key) == value, "manifest digest: " + key)
    return {"outer": outer, "worker": worker, "implementation": implementation,
            "sources": actual_records, "controls": controls}
...
def audit_inventory(bundle: Path, phase: str, limits: Limits) -> dict[str, Any]:
    ...
    need(manifest.get("schemaVersion") == f"feelm-service-v1-b1-{phase}-manifest/1", "manifest schema")
...
def readonly_mount_map(roots: Roots, phase: str) -> dict[str, Path]:
    paths = roots.sources()
    names = {"/app/run_service_v1_b1_gbt.py": "implementation/run_service_v1_b1_gbt.py",
             "/app/service_v1_b1_spark_worker.py": "implementation/service_v1_b1_spark_worker.py",
             "/contract/service-v1-b1-spark-runner.md": "implementation/service-v1-b1-spark-runner.md",
    ...
    if phase == "fit":
        ordered = ["manifest.json", "input-lock.json", "recovery-reference.json", "partition-identity.json", "command.json", "resource.json", "run.log"]
        controls = [roots.bundle("preflight") / name for name in ordered] + [roots.review("preflight")]
        result.update({f"/control/preflight/{i:02d}-{path.name}": path.resolve() for i, path in enumerate(controls)})
    elif phase == "score":
...
def audit_resource_and_commands(roots: Roots, phase: str, manifest: dict[str, Any], limits: Limits) -> dict[str, Any]:
    ...
    need(command.get("schemaVersion") == "feelm-service-v1-b1-command/1" and command.get("phase") == phase, "command schema/phase")
    ...
    paths = roots.sources()
    expected_mounts = readonly_mount_map(roots, phase)
    ...
    for index, (action, entry, stage, worker) in enumerate(zip(actions, sequence, stages, logs)):
        need(entry.get("order") == index and entry.get("workerAction") == stage.get("workerAction") == action, "worker action order")
        ...
        need(type(elapsed) in (int, float) and math.isfinite(elapsed) and 0 <= elapsed <= 5400 and end >= start, "stage timeout/duration")
        ...
        need(start >= created_at and 0 <= wall_seconds <= 5400
        ...
        need(observed.get("peakSource") in ("/sys/fs/cgroup/memory.peak", "/sys/fs/cgroup/memory/memory.max_usage_in_bytes"), "missing peak provenance")
        raw = worker.get("resourceObservation", {})
        need(raw.get("resourceStatus") == "PASS" and raw.get("peakBytes") == peak and raw.get("peakSource") == observed["peakSource"], "worker/log peak mismatch")
        ...
        for i in range(0, len(mount_args), 2):
            ...
            if "readonly" in pieces:
                ...
                need(not path.exists(), "scratch/staging cleanup incomplete")
    ...
    need(not bundle.with_name(bundle.name + "-failure.json").exists(), "phase failure record exists")
    need(not list(bundle.parent.glob("." + bundle.name + ".tmp-*")) and not list(bundle.parent.glob("." + bundle.name + ".*-scratch-*")), "stale phase staging/scratch")
...
def target_records(roots: Roots, phase: str) -> dict[str, dict[str, Any]]:
    ...
    key = "scoreSourceSetSha256" if phase == "score" else "trainingSourceSetSha256"
...
def audit_parent(roots: Roots, phase: str, expected_outer: str, expected_worker: str, limits: Limits) -> dict[str, Any]:
    ...
    need(review.get("schemaVersion") == "feelm-service-v1-b1-result-review/1" and review.get("status") == "PASS"
         and review.get("phase") == phase, "parent independent PASS review missing")
    expected = target_records(roots, phase)
    need(review.get("target") == expected, "parent review target pins changed/incomplete")
    ...
    if phase == "fit":
        ...
        audit_reference(roots, "fit", envelope["manifest"], lock, pins, earlier)
...
def audit_reference(roots: Roots, phase: str, manifest: dict[str, Any], lock: dict[str, Any],
                    pins: dict[str, Any], parent: dict[str, Any]) -> None:
    previous = "preflight" if phase == "fit" else "fit"
    reference = json_object(roots.bundle(phase) / (previous + "-reference.json"))
    expected = {"schemaVersion": f"feelm-service-v1-b1-{previous}-reference/1",
                previous + "Manifest": parent["manifestPin"], previous + "Review": parent["reviewPin"],
                "outerRunner": pins["outer"], "sparkWorker": pins["worker"], "implementationSetSha256": pins["implementation"]}
    ...
    if phase == "fit":
        expected.update({"reviewedArtifacts": pins["controls"], "trainingSourceSetSha256": lock["trainingSourceSetSha256"],
                         "fitInputSetSha256": lock["inputSetSha256"]})
        need(lock["trainingSourceSetSha256"] == parent["lock"]["trainingSourceSetSha256"], "preflight/fit common training set mismatch")
        # Phase-specific hashes must differ because only fit contains the preflight control files.
        need(lock["inputSetSha256"] != parent["lock"]["inputSetSha256"], "phase-specific input sets were collapsed")
        need(json_object(roots.bundle("fit") / "partition-identity.json") == json_object(roots.bundle("preflight") / "partition-identity.json"), "fit/preflight identity mismatch")
    else:
        ...
        expected.update({"modelInventory": pin(model_path, roots.logical(model_path)),
                         "modelFileSetSha256": model_inventory["inventorySha256"], "modelFiles": model_inventory["files"]})
    ...
    # Lists are published in path order with the review appended. Do not allow omitted siblings.
...
def dependency_fingerprint(roots: Roots, phase: str) -> dict[str, Any]:
    """Complete audited closure, including ancestor bundles and their sources.

    These records come from canonical paths, never from caller-supplied snapshot
    paths. Rebuilding the closure detects edits, additions and deletions before
    publishing even when a top-level manifest itself was left untouched.
    """
    ...
    source_names = TRAIN_NAMES | (SCORE_NAMES if phase == "score" else set())
    sources = roots.sources()
    files = {roots.logical(sources[name]): pin(sources[name]) for name in sorted(source_names) if name != "runtime/docker-image-id"}
    predecessor = roots.predecessor_failure()
    files[roots.logical(predecessor)] = pin(predecessor)
    bundles = {}
    for current in phases:
        bundle = roots.bundle(current)
        bundles[roots.logical(bundle)] = inventory(bundle)
        need(not bundle.with_name(bundle.name + "-failure.json").exists(), "dependency phase has failure record")
        need(not list(bundle.parent.glob("." + bundle.name + ".tmp-*"))
             and not list(bundle.parent.glob("." + bundle.name + ".*-scratch-*")), "dependency phase has stale scratch")
        if current != phase:
            files[roots.logical(roots.review(current))] = pin(roots.review(current))
    return {"files": files, "bundleInventories": bundles, "auditorImplementation": pin(Path(__file__).resolve()), "dockerImageId": IMAGE_ID}
...
def audit_phase(roots: Roots, phase: str, expected_outer: str, expected_worker: str,
    ...
    bundle = roots.bundle(phase)
    need(bundle.is_dir() and not bundle.is_symlink(), "canonical immutable bundle missing")
    ...
    parent = None
    if phase == "preflight":
        checks["recovery"] = audit_recovery_reference(roots, manifest, expected_outer, expected_worker, limits)
    else:
        parent = audit_parent(roots, "preflight" if phase == "fit" else "fit", expected_outer, expected_worker, limits)
        audit_reference(roots, phase, manifest, lock, pins, parent)
    ...
    # Re-hash the complete phase and all source/control pins after expensive recomputation.
    ...
    if phase == "preflight":
        audit_recovery_reference(roots, manifest, expected_outer, expected_worker, limits)
    need(dependency_fingerprint(roots, phase) == dependencies, "audited dependency changed during audit")
    return {"schemaVersion": "feelm-service-v1-b1-result-review/1", "phase": phase, "status": "PASS",
            ...
            "checks": checks, "evaluationTargetsRead": False, "modelFitPerformed": False,
            "readyForService": False, "scope": "Local immutable bundle integrity and specified numerical parity; no service acceptance or model quality verdict."}
...
def publish_review(roots: Roots, phase: str, review: dict[str, Any]) -> Path:
    need(review.get("status") == "PASS" and review.get("phase") == phase, "only a successful phase audit can publish")
    need(review.get("target") == target_records(roots, phase), "bundle changed before review publication")
    need(review.get("dependencyFingerprint") == dependency_fingerprint(roots, phase), "audited dependency changed before review publication")
    destination = roots.review(phase)
    need(not destination.exists(), "immutable result review already exists")
    ...
    try:
        ...
        need(review["dependencyFingerprint"] == dependency_fingerprint(roots, phase), "audited dependency changed immediately before publication")
