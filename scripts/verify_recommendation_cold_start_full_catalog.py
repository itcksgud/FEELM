...
def main() -> None:
    ...
    for r in m["artifacts"].values():
        q=Path(r["path"]); assert q.is_file() and q.stat().st_size==r["bytes"] and sha256(q)==r["sha256"]
    lock=json.loads(Path(m["artifacts"]["protocol_lock"]["path"]).read_text(encoding="utf-8"))
    assert lock["protocol_hash"]==hashlib.sha256(canonical_bytes(lock["protocol"])).hexdigest()
    result=json.loads(Path(m["artifacts"]["evaluation_result"]["path"]).read_text(encoding="utf-8"))
