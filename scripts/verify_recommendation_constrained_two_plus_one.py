...
def main():
 ...
 for r in m['artifacts'].values():q=Path(r['path']);assert q.is_file() and q.stat().st_size==r['bytes'] and sha256(q)==r['sha256']
 lock=json.loads(Path(m['artifacts']['lock']['path']).read_text());assert lock['protocol_hash']==hashlib.sha256(canonical_bytes(lock['protocol'])).hexdigest();assert lock['selected'] is None
 result=json.loads(Path(m['artifacts']['result']['path']).read_text());assert result['selected'] is None and result['positive_injection'] is False;assert result['paired_ci']['status']=='DIAGNOSTIC_FAILED_SELECTION_BUDGET'
