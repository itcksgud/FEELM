#!/usr/bin/env python3
"""Fail-closed validator for the REC-EV-023E execution contract."""
...
CONTRACT = ROOT / "docs/recommendation/contracts/rec-ev-023e-korean-origin-transfer.json"
JOINT = ROOT / "docs/recommendation/contracts/rec-ev-023ef-joint-transfer-design.json"
EXPECTED_CANONICAL_SHA256 = "38c2c676f2c8ec3d962b9a6b631fba62671676c28f9a987a76d737af742b913b"
...
def validate_contract(contract: Mapping[str, Any]) -> dict[str, Any]:
    require(canonical_sha(contract) == EXPECTED_CANONICAL_SHA256, "canonical REC-EV-023E contract hash drift")
    require(contract.get("contract_id") == "rec-ev-023e-korean-origin-transfer-v1", "contract identity drift")
    require(contract.get("evidence_id") == "REC-EV-023E", "evidence identity drift")
    ...
    require(binding == {
        ...
        "inherited_sections": ["roles_and_reader", "universe", "common_design", "experiments.REC-EV-023E", "statistics", "decision", "claim_boundary", "resume", "invariants"],
    ...
    require(contract["execution_delta"] == {
        "profile_control_partition": "FOR_EACH_USER_PANEL_HASH_ORDER_NON_KOREAN_POOL_THEN_PROFILE_FIRST14_AND_CONTROL_NEXT10_WITHOUT_OVERLAP",
    ...
    require(contract["implementation_artifacts"] == [
        ...
        "scripts/run_rec_ev_023ef_transfer.py", "scripts/validate_rec_ev_023e_contract.py",
    ...
    experiment = joint["experiments"]["REC-EV-023E"]
    ...
    require((preflight["expected_users"], preflight["expected_unique_selected_targets"], preflight["expected_selected_target_memberships"]) == (319, 494, 12760), "preflight evidence drift")
    cohort = contract["cohort"]
    require(cohort["profile_domain"] == "NON_KOREAN_STRUCTURED" and cohort["target_domain"] == "KOREAN_ORIGIN_STRUCTURED", "domain drift")
    require(cohort["control_domain"] == "NON_KOREAN_STRUCTURED_DISJOINT_FROM_PROFILE", "control disjointness drift")
    require((cohort["profile_n"], cohort["target_n"], cohort["control_n"], cohort["panels"]) == (14, 10, 10, 4), "panel size drift")
    require((cohort["minimum_profile_control_ratings"], cohort["minimum_target_ratings"]) == (24, 10), "cohort floor drift")
    require(cohort["profile_control_salts"] == joint["common_design"]["panel_salts"]["REC-EV-023E"]["profile_control_order"], "profile/control salt drift")
    require(cohort["target_salts"] == joint["common_design"]["panel_salts"]["REC-EV-023E"]["target_order"], "target salt drift")
    ...
    require(contract["claim_boundary"]["allowed"] == joint["claim_boundary"]["allowed_e"], "allowed claim drift")
    ...
    return {"status": "PASS_REC_EV_023E_CONTRACT", "primary_family": 108}
