...
public final class C6LoopbackGuard {
    private final String serverAddress;

    public C6LoopbackGuard(@Value("${server.address:127.0.0.1}") String serverAddress) {
    ...
    public void validateBind() {
        if (!isLoopback(serverAddress)) {
            throw new IllegalStateException("C6 local experiment requires an explicit loopback server.address");
        }
    }
